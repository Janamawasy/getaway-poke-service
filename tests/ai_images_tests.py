...
def test_generate_battle_image(poke1_name, poke2_name):
    response = client.post(f'/ai/images/battle/?poke1_name={poke1_name}&poke2_name={poke2_name}')
...
@pytest.mark.parametrize("poke1_name, poke2_name", [("ditto", "eevee"), ("bulbasaur", "hoothoot")])
def test_generate_evolution_image(poke1_name, poke2_name):
    response = client.post(f'/ai/images/evolve/?poke1_name={poke1_name}&poke2_name={poke2_name}')
    assert response.status_code == 200, f'Expected status_code 200 but got {response.status_code} - {response.text}'
